...
<head>
    ...
    <script>
        function submitForm() {
            document.getElementById('popup-modal').close(); // Tutup modal sebelum submit
            setTimeout(() => {
                document.getElementById('kuisionerForm').submit(); // Kirim form setelah modal tertutup
            }, 100);
        }
    </script>
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    ...
    <div class="flex flex-col md:flex-row max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
        <div class="w-full md:w-1/2 bg-cover bg-center h-40 md:h-auto" style="background-image: url('gambar3.webp');">
        </div>
        <div class="w-full md:w-1/2 p-6">
            <h1
                class="text-xl sm:text-2xl md:text-3xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-blue-950 to-green-600 text-center mb-4">
                KUISIONER PARTISIPAN
            </h1>

            <p class="text-center text-sm text-gray-600 mb-4">
                Silakan isi formulir kuisioner di bawah ini untuk dapat mengunduh sertifikat Anda. Pastikan semua
                informasi diisi dengan benar.
            </p>

            <!-- ✅ Tambahkan id="kuisionerForm" -->
            <form id="kuisionerForm" action="submission_results.php" method="POST" class="flex flex-col">
                ...
                <button type="button"
                    class="w-full p-2 mt-3 bg-gradient-to-r from-green-600 to-blue-950 text-white rounded-md 
