...
<body class="bg-gray-100">
    ...
    <div class=" grow overflow-y-auto p-8 bg-gray-100  ">
        <h1 class="text-3xl font-bold text-gray-700">Tabel Fasilitas</h1>

        <div class="mt-8 w-40">
            <a href="<?= Config::BASE_URL . '/facilities/create' ?>" class="flex facilitys-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-green-600 to-blue-600
            ...
            <table class="w-full border border-gray-300 rounded-lg text-sm">
                ...
                <tbody>
                    <?php if (empty($facilities)): ?>
                        <tr>
                            <td colspan="8" class="text-center text-gray-700 p-4">Fasilitas Tidak Ditemukan</td>
                        </tr>
                    <?php else: ?>

                        <?php foreach ($facilities as $facility): ?>
                            <tr class='text-center text-gray-700 hover:bg-gray-200 transition'>
                                <td class='border border-gray-300 px-4 py-2'><?= htmlspecialchars($facility['id']) ?></td>
                                <td class='border border-gray-300 px-4 py-2'><?= htmlspecialchars($facility['name']) ?></td>
                                <td class='border border-gray-300 px-4 py-2 flex justify-center space-x-2'>


                                <td class='border border-gray-300 px-4 py-2 flex justify-center space-x-2'>
                                    <a href="<?= Config::BASE_URL . "/facilities/" . $facility['id'] . "/edit" ?>"
                                        class='bg-blue-500 text-white px-3 py-1 rounded-md hover:bg-blue-600 transition shadow-md text-sm whitespace-nowrap'>
                                        Edit
                                    </a>
                                   <form action="<?= Config::BASE_URL . "/facilities/" . $facility['id']?>" method='POST'
